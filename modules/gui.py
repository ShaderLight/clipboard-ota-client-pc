...
class Client():
    ...
    def render_gui(self):
        ...
        get_button = tk.Button(window, text='Get clipboard', command=lambda: self.request_handler('get'))
        get_button.grid(column=0, row=2, padx=5, pady=5)

        post_button = tk.Button(window, text='Send clipboard', command=lambda: self.request_handler('post'))
